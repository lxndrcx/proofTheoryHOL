...
/\ (!A D. (N ((Not A) INSERT D) Bot) ==> (N D A))`; (* Classical Absurdity Rule *)
...
/\ (!A D. (Nd D Bot) ==> (Nd (D DELETE (Not A)) A))`; (* Classical Absurdity Rule *)
  ...
  >- (Induct_on `N` >>
      rw[Nd_rules]
        ...
        >- (`Nd (D ∪ ((A INSERT D1) DELETE A) ∪ ((B INSERT D2) DELETE B)) C`
            ...
            rw[SUBSET_DEF])
        >- (`Nd ((Not A INSERT D) DELETE Not A) A` by metis_tac[Nd_bot] >>
            fs[DELETE_DEF] >>
            irule Nd_lw_SUBSET >>
        ...
        >- (`N (A INSERT (D1 DELETE A)) C`
            ...
            `N (B INSERT (D2 DELETE B)) C`
              by (irule N_lw_SUBSET >>
                  ...
                  rw[DELETE_DEF,SUBSET_DEF]) >>
            metis_tac[N_ore])
        >- (`N (Not A INSERT D) Bot` by metis_tac[N_lw] >>
            irule N_bot >>
            Cases_on `Not A ∈ D`
              >- (`?D'. (D = Not A INSERT D') /\ Not A NOTIN D'` 
                    by metis_tac[DECOMPOSITION] >>
                  fs[] >> rw[])
              >- fs[DELETE_NON_ELEMENT_RWT]))
QED
...
(!(A:'a formula) Γ Δ. (A <: Γ) /\ FINITE_BAG Γ /\
                      (A <: Δ) /\ FINITE_BAG Δ
                      ==> G Γ Δ) (* Ax *)
/\ (∀ Γ Δ. (Bot <: Γ) /\ FINITE_BAG Γ /\ FINITE_BAG Δ ==> G Γ Δ) (* LBot *)
/\ (!A Γ Δ. (G ({|A;A|} + Γ) Δ) ==> G ({|A|} + Γ) Δ) (* Left Contraction *)
/\ (!A Γ Δ. (G Γ ({|A;A|} + Δ)) ==> G Γ ({|A|} + Δ)) (* Right Contraction *)
/\ (!A B Γ Δ. (G (BAG_INSERT A Γ) Δ)
   ==> (G (BAG_INSERT (A And B) Γ) Δ)) (* Left And 1 *)
/\ (!A B Γ Δ. (G (BAG_INSERT B Γ) Δ)
   ==> (G (BAG_INSERT (A And B) Γ) Δ)) (* Left And 2 *)
/\ (!A B Γ Δ. (G Γ (BAG_INSERT A Δ)) /\ (G Γ (BAG_INSERT B Δ))
    ==> G Γ (BAG_INSERT (A And B) Δ)) (* Right And *)
/\ (!A B Γ Δ. (G (BAG_INSERT A Γ) Δ)
    /\ (G (BAG_INSERT B Γ) Δ)
    ==> G ((BAG_INSERT (A Or B) Γ)) Δ) (* Left Or *)
/\ (!A B Γ Δ. (G Γ (BAG_INSERT A Δ))
    ==> G Γ (BAG_INSERT (A Or B) Δ)) (* Right Or 1 *)
/\ (!A B Γ Δ. (G Γ (BAG_INSERT B Δ)) 
    ==> G Γ (BAG_INSERT (A Or B) Δ)) (* Right Or 2 *)
/\ (!A B Γ Δ. (G Γ (BAG_INSERT A Δ)) /\ (G (BAG_INSERT B Γ) Δ)
    ==> G ((BAG_INSERT (A Imp B) Γ)) Δ) (* Left Imp *)
/\ (!A B Γ Δ. (G (BAG_INSERT A Γ) (BAG_INSERT B Δ))
    ==> G Γ (BAG_INSERT (A Imp B) Δ)) (* Right Imp *)
/\ (∀A Γ Γ' Δ Δ'. (G Γ (BAG_INSERT A Δ)) ∧ (G (BAG_INSERT A Γ') Δ') 
    ==> G (Γ ⊎ Γ') (Δ ⊎ Δ'))` (* Cut_cs *)

val [G_ax, G_bot, G_lc, G_rc, G_landl, G_landr, G_rand,
...
Theorem G_FINITE:
  !Γ Δ. G Γ Δ ==> FINITE_BAG Γ /\ FINITE_BAG Δ
Proof Induct_on `G` >> simp[]
QED
...
Proof
  Induct_on `G` >> rw[]
  >- metis_tac[G_ax,SUB_BAG, BAG_IN]
  >- metis_tac[G_bot,SUB_BAG,BAG_IN]
  >- (‘∃Γ₂. Γ' = {|A|} ⊎ Γ₂’
        by (qexists_tac ‘Γ' - {|A|}’ >> irule SUB_BAG_DIFF_EQ >>
            metis_tac[SUB_BAG_UNION_down]) >>
      rw[] >> irule G_lc >> first_x_assum irule >> fs[])
  >- metis_tac[G_rc,SUB_BAG,BAG_IN]
  ...
  >- (‘∃Γ₂. Γ' = BAG_INSERT (A Imp B) Γ₂’
        by (qexists_tac ‘Γ' - {|A Imp B|}’ >> fs[BAG_INSERT_UNION] >>
            irule SUB_BAG_DIFF_EQ >> metis_tac[SUB_BAG_UNION_down]) >>
      rw[] >> fs[SUB_BAG_INSERT] >> irule G_limp >> conj_tac >>
      first_x_assum (fn th => irule th >> fs[SUB_BAG_INSERT] >> NO_TAC))
...
Theorem G_rw:
  ∀Γ Δ. G Γ Δ ==> ∀Δ'. Δ ≤ Δ' /\ FINITE_BAG Δ' ==> G Γ Δ'
Proof
  Induct_on `G` >>
  rw[G_rules]
  >- metis_tac[G_ax,SUB_BAG,BAG_IN]
  >- (‘∃Δ₂. Δ' = {|A|} ⊎ Δ₂’
        by (qexists_tac ‘Δ' - {|A|}’ >> irule SUB_BAG_DIFF_EQ >>
            metis_tac[SUB_BAG_UNION_down]) >>
      rw[] >> irule G_rc >> first_x_assum irule >> fs[])
  >- (‘∃Δ₂. Δ' = BAG_INSERT (A And B) Δ₂’ 
         by (qexists_tac ‘Δ' - {|A And B|}’ >> 
             fs[BAG_INSERT_UNION] >> 
             irule SUB_BAG_DIFF_EQ >> 
             metis_tac[SUB_BAG_UNION_down]) >> 
       rw[] >> irule G_rand >> fs[SUB_BAG_INSERT])
  >- (‘∃Δ₂. Δ' = BAG_INSERT (A Or B) Δ₂’ 
        by (qexists_tac ‘Δ' - {|A Or B|}’ >> 
            fs[BAG_INSERT_UNION] >> 
            irule SUB_BAG_DIFF_EQ >> 
            metis_tac[SUB_BAG_UNION_down]) >>
      rw[] >> fs[SUB_BAG_INSERT] >> irule G_rorl >> 
      first_x_assum irule >> fs[SUB_BAG_INSERT])
  >- (‘∃Δ₂. Δ' = BAG_INSERT (A Or B) Δ₂’ 
        by (qexists_tac ‘Δ' - {|A Or B|}’ >> 
            fs[BAG_INSERT_UNION] >> 
            irule SUB_BAG_DIFF_EQ >> 
            metis_tac[SUB_BAG_UNION_down]) >> 
      rw[] >> fs[SUB_BAG_INSERT] >> irule G_rorr >> 
      first_x_assum irule >> fs[SUB_BAG_INSERT])
  >- (irule G_limp >> fs[SUB_BAG_INSERT])
  >- (‘∃Δ₂. Δ' = BAG_INSERT (A Imp B) Δ₂’ 
        by (qexists_tac ‘Δ' - {|A Imp B|}’ >> 
            fs[BAG_INSERT_UNION] >> 
            irule SUB_BAG_DIFF_EQ >> 
            metis_tac[SUB_BAG_UNION_down]) >> 
      rw[] >> fs[SUB_BAG_INSERT] >> irule G_rimp >> 
      first_x_assum irule >> fs[SUB_BAG_INSERT])
  >- (rename [‘Δ₁ ⊎ Δ₂ ≤ Δ₃’] >>
      ‘∃Δ₀. Δ₃ = Δ₀ ⊎ Δ₁ ⊎ Δ₂’
        by metis_tac[SUB_BAG_EXISTS, COMM_BAG_UNION, ASSOC_BAG_UNION] >>
...
Theorem G_runibag:
  ∀Γ A. G Γ Δ <=> G Γ (unibag Δ)
Proof
 rw[] >> EQ_TAC
 >- (`∀Δ. FINITE_BAG Δ ==> ∀Γ. G Γ Δ ==> G Γ (unibag Δ)`
      suffices_by metis_tac[G_FINITE] >>
      gen_tac >>
      Induct_on `BAG_CARD Δ` >>
      rw[]
      >- (`Δ = {||}` by metis_tac[BCARD_0] >>
          fs[])
      >- (Cases_on `unibag Δ = Δ` >- fs[] >>
          drule_then strip_assume_tac unibag_DECOMPOSE >>
          rename [`Δ = {|B;B|} ⊎ Δ0`,`SUC n = BAG_CARD Δ`] >>
          `G Γ ({|B|} ⊎ Δ0)` by metis_tac[G_rc] >>
          `BAG_CARD ({|B|} ⊎ Δ0) = n` by fs[BAG_CARD_THM] >>
          `FINITE_BAG ({|B|} ⊎ Δ0)` by fs[] >>
          metis_tac[unibag_AA_A]))
  >- metis_tac[unibag_SUB_BAG,G_rw,G_FINITE,unibag_FINITE]
QED

Theorem G_cut_cs:
  ∀Γ Δ A. G Γ (BAG_INSERT A Δ) /\ G (BAG_INSERT A Γ) Δ
          ==> G Γ Δ
Proof
  rw[] >>
  `G (Γ ⊎ Γ) (Δ ⊎ Δ)` by metis_tac[G_cut] >>
  `G (unibag Γ) (unibag Δ)` suffices_by metis_tac[G_unibag,G_runibag] >>
  `G (unibag (Γ ⊎ Γ)) (unibag (Δ ⊎ Δ))` by metis_tac[G_unibag,G_runibag] >>
  metis_tac[unibag_UNION,BAG_MERGE_IDEM]
QED

Theorem G_stab_lnot:
  ∀Γ A Δ. G (BAG_INSERT (Not A) Γ) Δ <=> G Γ (BAG_INSERT (A) Δ)
Proof
  rw[] >> EQ_TAC
  >- (rw[] >>
      `(G Γ (BAG_INSERT ((Not Not A) Imp A) (BAG_INSERT A Δ))) /\
       (G (BAG_INSERT ((Not Not A) Imp A) Γ) (BAG_INSERT A Δ))`
         suffices_by metis_tac[G_cut_cs] >>
      conj_tac
      >- (irule G_rimp >>
          rw[Once Not_def] >>
          irule G_limp >>
          conj_tac
          >- (rw[Not_def] >> irule G_rimp >>
              rw[BAG_INSERT_commutes] >>
              irule G_ax >>
              conj_tac >- metis_tac[G_FINITE,FINITE_BAG_THM] >>
              conj_tac >- metis_tac[G_FINITE,FINITE_BAG_THM] >>
              qexists_tac `A` >>
      ...
      >- (irule G_limp >>
          conj_tac
          >- (rw[Once Not_def] >> irule G_rimp >>
              irule G_rw >>
              conj_tac >- metis_tac[G_FINITE,FINITE_BAG_THM] >>
              qexists_tac `Δ` >>
              simp[] >>
              metis_tac[SUB_BAG_INSERT_I,SUB_BAG,BAG_IN])
          >- (irule G_ax >>
              conj_tac >- metis_tac[G_FINITE,FINITE_BAG_THM] >>
              conj_tac >- metis_tac[G_FINITE,FINITE_BAG_THM] >>
              qexists_tac `A` >>
              conj_tac >> metis_tac[BAG_IN_BAG_INSERT])))
  >- (rw[Not_def] >> irule G_limp >>
      conj_tac >- simp[] >>
      irule G_bot >>
      conj_tac >- metis_tac[G_FINITE,FINITE_BAG_THM] >>
      conj_tac >- metis_tac[G_FINITE,FINITE_BAG_THM] >>
